...
export const TableUnidades: React.FC<TableUnidadesProps> = ({ data, ariaLabel = "Tabela Unidades" }) => {
  ...
  return (
    <div className="w-full">
      {/* Cabeçalho específico Unidades */}
      <div className="bg-purple-50 border justify-center border-purple-200 rounded-t p-2 flex items-center gap-2">
        <Package2 className="w-4 h-4 text-purple-600" />
        <span className="font-bold text-purple-700">SEPARAÇÃO POR UNIDADES</span>
      </div>
      ...
      >
            ...
            {visibleColumns.map((col: ColumnConfig) => (
              <th
                ...
                className={cn(
                  ...
                  col.key === "units" ? "text-purple-700 bg-purple-200" : "text-gray-700"
                )}
                tabIndex={0}
        ...
        <tbody>
          {data.map((row, idx) => {
            ...
            const hasUnits = row.units && row.units > 0;
            return (
              <tr
                key={idx}
                className={cn(
                  "even:bg-purple-25 hover:bg-purple-50 focus-within:bg-purple-100",
                  !isGreenBelt && "font-bold",
                  hasUnits && "bg-purple-50"
                )}
                tabIndex={0}
                  ...
                  return (
                    <td
                      ...
                      className={cn(
                        ...
                        col.key === "units" && "bg-purple-50 font-bold text-purple-700"
                      )}
                      tabIndex={0}
